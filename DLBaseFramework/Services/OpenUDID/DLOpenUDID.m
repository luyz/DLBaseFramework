...
# error                                                         ^ read comments above, fix accordingly, and remove this #error line
    ...
    if (_openUDID==nil) {
        ...
        CC_LONG	md5Length = (CC_LONG)[[NSString stringWithUTF8String:cStr] length];
        CC_MD5( cStr, md5Length, result );
        CFRelease(cfstring);
        CFRelease(uuid);
...
//
+ (NSString*) value {
    return [DLOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [DLOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [DLOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [DLOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
